...
router.post('/', authMiddleware, async (req, res) => {
    ...
    let sessionIds = [];
    if (scenario_id) {
        const scenario = db.prepare('SELECT matched_session_ids FROM scenarios WHERE id = ?').get(scenario_id);
        if (scenario)
            sessionIds = JSON.parse(scenario.matched_session_ids || '[]');
    }
    if (sessionIds.length === 0) {
        sessionIds = db.prepare('SELECT id FROM sessions').all().map(r => r.id);
    }
    const runId = uuid();
...
router.post('/:id/process', authMiddleware, async (req, res) => {
    ...
    const config = db.prepare('SELECT * FROM analysis_configs WHERE id = ?').get(run.config_id);
    if (!config)
    ...
    const processed = db.prepare('SELECT DISTINCT session_id FROM analysis_results WHERE run_id = ?').all(run.id);
    const processedSet = new Set(processed.map((r) => r.session_id));
    ...
    const dimensions = dimensionIds.length > 0
        ? dimensionIds.map(id => db.prepare('SELECT * FROM dimensions WHERE id = ?').get(id)).filter(Boolean)
        : db.prepare('SELECT * FROM dimensions WHERE enabled = 1 ORDER BY sort_order').all();
...
router.get('/:id/results', (req, res) => {
    ...
    const dimensionId = req.query.dimension_id;
    ...
    const total = db.prepare(`SELECT COUNT(*) as cnt FROM analysis_results WHERE run_id = ?`).get(req.params.id);
