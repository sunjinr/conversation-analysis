...
router.get('/summary', (req, res) => {
    ...
    const totalRuns = db.prepare('SELECT COUNT(*) as cnt FROM analysis_runs').get();
    const openTasks = db.prepare("SELECT COUNT(*) as cnt FROM tasks WHERE status = 'open'").get();
    const claimedTasks = db.prepare("SELECT COUNT(*) as cnt FROM tasks WHERE status = 'claimed'").get();
    const resolvedTasks = db.prepare("SELECT COUNT(*) as cnt FROM tasks WHERE status = 'resolved'").get();
    const latestRun = db.prepare('SELECT * FROM analysis_runs ORDER BY created_at DESC LIMIT 1').get();
    // Satisfaction score from dissatisfaction data
    const dissatisfied = db.prepare("SELECT COUNT(*) as cnt FROM sessions WHERE dissatisfaction_info LIKE '%点了不满意%'").get();
    res.json({
...
router.get('/satisfaction', (req, res) => {
  ...
  `).all();
    ...
    const events = db.prepare('SELECT * FROM satisfaction_events ORDER BY event_date').all();
    const eventMap = {};
    for (const e of events)
        eventMap[e.event_date] = e;
