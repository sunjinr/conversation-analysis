...
export function initDB() {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS sessions (
      ...
      human_conversation TEXT,
      dissatisfaction_info TEXT,
      session_date TEXT,
    ...
    CREATE TABLE IF NOT EXISTS satisfaction_events (
      ...
      task_resolved_ids TEXT DEFAULT '[]',
      notes TEXT DEFAULT '',
      created_at TEXT DEFAULT (datetime('now'))
    );
  `)
  ...
  // Seed default dimensions if not exists
  ...
  if (dimCount.cnt === 0) {
    ...
    const stmt = db.prepare(
      'INSERT INTO dimensions (id, name, definition, categories_json, auto_discover, sub_skill_ref, sort_order, enabled, created_by) VALUES (?, ?, ?, ?, 1, \'\', ?, 1, \'admin-001\')'
    )
    for (const d of dims) {
      stmt.run(d.id, d.name, d.definition, JSON.stringify(d.categories), d.sort_order)
    }
  }
