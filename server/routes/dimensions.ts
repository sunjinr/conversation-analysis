...
router.post('/', authMiddleware, (req: AuthRequest, res) => {
  ...
  const id = uuid()
  const maxOrder = db.prepare('SELECT MAX(sort_order) as m FROM dimensions').get() as any
  db.prepare(`INSERT INTO dimensions (id, name, definition, categories_json, auto_discover, sub_skill_ref, sort_order, enabled, created_by)
    VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?)`).run(
    id, name, definition, JSON.stringify(categories || []),
    auto_discover ? 1 : 0, sub_skill_ref || '', (maxOrder.m || 0) + 1, req.user!.id
  )
  res.json({ id })
})
